...
代码如下：
[参考文章](https://leetcode-cn.com/problems/linked-list-cycle-ii/solution/tu-jie-kuai-man-zhi-zhen-ji-qiao-yuan-li-5tz0/)
```JavaScript
/**
 * @param {ListNode} head
 * @return {ListNode}
...
var detectCycle = function(head) {
  ...
  while (fast && fast.next) { // 
    ...
    if (slow == fast) { // 包含环
      // 任一一节点指向头部那个头节点,不在环内.
      fast = head;
      // 同步向前进,因为距离都是k-m,所以下面循环的fast指针和slow指针一定相遇.
      while (fast != slow) {
        fast = fast.next;
        slow = slow.next;
      }
