...
## 【个人对二分法的总结】devin
二分法的作用对象有两种区间，第一种是左闭右闭区间[left, right]，第二种是左闭右开区间[left, right)。
- 左闭右闭区间[left, right]
  - 它的while条件是left <= right。为什么可以取得等号呢？因为left = right是可以取得到的（left 和 right都包含在闭区间内）。在缩小区间的时候，left和right都是可以不等于mid的，可以通过加1或者减1来加速搜索目标对象。
- 左闭右开区间[left, right)
  - 它的while条件是left < right。为什么不可以取得等号呢？因为left虽然在区间内，但是right不在区间内。在缩小区间的时候，left是可以不等于mid的(因为是左闭区间，所以left是可以进一步缩小的，left可以做加1处理)，但是right是等于mid的（因为是右开区间，所以right不能做进一步处理，只能等于mid）
