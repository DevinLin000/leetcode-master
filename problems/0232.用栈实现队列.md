...
MyQueue.prototype.pop = function() { 
  // 如果输出栈不为空，就直接返回输出栈栈顶元素；如果输出栈为为空，就要一次性全部导入输入栈目前已有的元素
...
MyQueue.prototype.peek = function() {
  // 这里的this.pop() 运行的时候，实质上会把stackIn绑定给this，这里是把stackIn当作一个数组操作了
  const x = this.pop() // 这里不能使用this.stachIn.peek(). 因为栈中的peek方法是返回栈顶元素的
